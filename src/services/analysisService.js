...
/**
 * Analyzes an image using AI (OpenAI or Gemini) to generate a Mermaid diagram.
 * @param {File} imageFile 
 * @returns {Promise<string>} Mermaid diagram string
 */
export const generateMermaidFromImage = async (imageFile) => {
    const defaultModel = import.meta.env.VITE_DEFAULT_MODEL || 'gpt';
    const openaiKey = import.meta.env.VITE_OPENAI_API_KEY;
    const geminiKey = import.meta.env.VITE_GEMINI_API_KEY;

    if (defaultModel === 'gemini' && geminiKey) {
        console.log("analysisService: Using Gemini API for Mermaid generation");
        return generateMermaidWithGemini(imageFile, geminiKey);
    } else if (defaultModel === 'gpt' && openaiKey) {
        console.log("analysisService: Using OpenAI API for Mermaid generation");
        return generateMermaidWithOpenAI(imageFile, openaiKey);
    } else {
...
export const convertMermaidToFlow = async (mermaidCode) => {
    ...
    if (defaultModel === 'gemini' && geminiKey) {
        ...
        return convertToFlowWithGemini(mermaidCode, geminiKey);
    } else if (defaultModel === 'gpt' && openaiKey) {
        console.log("analysisService: Using OpenAI API for Flow conversion");
        return convertToFlowWithOpenAI(mermaidCode, openaiKey);
    } else {
        console.log("analysisService: No API key found, using mock data");
        return new Promise((resolve) => {
            setTimeout(() => {
